...
from utils.common import AverageTimer
from utils.common import compute_epipolar_error
from utils.common import compute_pixel_error
from utils.common import compute_pose_error
from utils.common import download_base_files
from utils.common import download_test_images
from utils.common import error_colormap
from utils.common import estimate_pose
from utils.common import make_matching_plot
from utils.common import pose_auc
from utils.common import read_image
from utils.common import read_image_with_homography
from utils.common import rotate_intrinsics
from utils.common import rotate_pose_inplane
from utils.common import scale_intrinsics
from utils.common import weights_mapping
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_homography",
        type=str,
        default="assets/coco_test_images_homo.txt",
        help="Path to the list of image pairs and corresponding homographies",
    )
    parser.add_argument(
        "--input_dir",
        type=str,
        default="assets/coco_test_images/",
        help="Path to the directory that contains the images",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="dump_homo_pairs/",
    ...
    parser.add_argument("--superglue", default="coco_homo", help="SuperGlue weights")
    parser.add_argument(
        "--max_keypoints",
        type=int,
        default=1024,
    ...
    try:
        ...
        if os.path.isfile(opt.superglue) and (
            os.path.splitext(opt.superglue)[-1] in [".pt", ".pth"]
        ):
    ...
    for i, info in enumerate(homo_pairs):
        ...
        viz_eval_path = output_dir / "{}_evaluation.{}".format(stem0, opt.viz_extension)
        ...
        if not (do_match or do_eval or do_viz or do_viz_eval):
            ...
            continue
        image0, image1, inp0, inp1, scales0, homo_matrix = read_image_with_homography(
            input_dir / image_name, homo_matrix, device, opt.resize, 0, opt.resize_float
        )

        if image0 is None or image1 is None:
            print("Problem reading image pair: {}".format(input_dir / image_name))
            exit(1)
        ...
        ma_0, ma_1, miss_0, miss_1 = torch_find_matches(
            kp0_torch,
            kp1_torch,
            torch.from_numpy(homo_matrix).to(kp0_torch.device),
            dist_thresh=3,
            n_iters=3,
        )
        ...
        if do_eval:
            ...
            corner_points_gt = cv2.perspectiveTransform(
                np.reshape(corner_points, (-1, 1, 2)), homo_matrix
            ).squeeze(1)
    ...
    if opt.eval:
        ...
        aucs_dlt = [100.0 * yy for yy in aucs_dlt]
        aucs_ransac = [100.0 * yy for yy in aucs_ransac]
        prec = 100.0 * np.mean(precisions)
        rec = 100.0 * np.mean(recall)
