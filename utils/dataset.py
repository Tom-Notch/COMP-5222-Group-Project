...
from .preprocess_utils import get_perspective_mat
from .preprocess_utils import resize_aspect_ratio
...
class COCO_loader(Dataset):
    def __init__(self, dataset_params, typ="train"):
        ...
        self.json_path = os.path.join(
            self.dataset_path, "annotations", "instances_{}2017.json".format(typ)
        )
        ...
        if self.apply_aug:
            ...
            self.aug_list = [
                alb.OneOf(
                    [
                        alb.RandomBrightness(limit=0.4, p=0.6),
                        alb.RandomContrast(limit=0.3, p=0.7),
                    ],
                    p=0.6,
                ),
    ...
    def __getitem__(self, index: int):
        ...
        homo_matrix = get_perspective_mat(
            self.aug_params["patch_ratio"],
            width // 2,
            height // 2,
            self.aug_params["perspective_x"],
            self.aug_params["perspective_y"],
            self.aug_params["shear_ratio"],
            self.aug_params["shear_angle"],
            self.aug_params["rotation_angle"],
            self.aug_params["scale"],
            self.aug_params["translation"],
        )
        warped_image = cv2.warpPerspective(image.copy(), homo_matrix, (width, height))
        if resize:
            orig_resized = cv2.resize(
                image, (self.config["image_width"], self.config["image_height"])
            )
            warped_resized = cv2.resize(
                warped_image, (self.config["image_width"], self.config["image_height"])
            )
        else:
            orig_resized = image
            warped_resized = warped_image
        if self.apply_aug:
            orig_resized, warped_resized = self.apply_augmentations(
                orig_resized, warped_resized
            )
        homo_matrix = scale_homography(
            homo_matrix,
            height,
            width,
...
class COCO_valloader(Dataset):
    def __init__(self, dataset_params):
        ...
        self.txt_path = str(
            Path(__file__).parent.parent / "assets/coco_val_images_homo.txt"
        )
    ...
    def __getitem__(self, index: int):
        ...
        image = cv2.imread(
            os.path.join(self.images_path, image_name), cv2.IMREAD_GRAYSCALE
        )
        height, width = image.shape[0:2]
        warped_image = cv2.warpPerspective(image.copy(), homo_matrix, (width, height))
        orig_resized = cv2.resize(
