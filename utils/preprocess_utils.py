...
def get_translation_mat(image_height, image_width, trans, transformed_corners):
    ...
    right_bottom_min = np.min(
        np.array([image_width, image_height]) - transformed_corners, axis=0
    )
...
def get_perspective_mat(
    ...
    patch_bound_w, patch_bound_h = int(patch_ratio * image_width), int(
        patch_ratio * image_height
    )
    patch_corners = np.array(
        [[0, 0], [0, patch_bound_h], [patch_bound_w, patch_bound_h], [patch_bound_w, 0]]
    ).astype(np.float32)
    ...
    shear_matrix = (
        get_rotmat(-shear_angle_value, as_3d=True, center_x=center_x, center_y=center_y)
        @ np.diag([shear_x, shear_y, 1])
        @ get_rotmat(
            shear_angle_value, as_3d=True, center_x=center_x, center_y=center_y
        )
    )
    ...
    scaled_rotation_matrix = get_rotmat(
    ...
    trans_patch_corners = cv2.perspectiveTransform(
        np.reshape(patch_corners, (-1, 1, 2)), homography_matrix
    ).squeeze(1)
    translation_matrix = get_translation_mat(
        image_height, image_width, trans, trans_patch_corners
    )
...
def warp_keypoints(keypoints, homography_mat):
    source = torch.cat(
        [keypoints, torch.ones(len(keypoints), 1).to(keypoints.device)], dim=-1
    )
...
def torch_find_matches(
    src_keypoints1, src_keypoints2, homography, dist_thresh=3, n_iters=1
):
    match_list_1, match_list_2 = torch.empty(
        0, dtype=torch.int64, device=src_keypoints1.device
    ), torch.empty(0, dtype=torch.int64, device=src_keypoints2.device)
    missing_indices_1 = torch.arange(
        len(src_keypoints1), device=src_keypoints1.device, dtype=torch.long
    )
    missing_indices_2 = torch.arange(
        len(src_keypoints2), device=src_keypoints2.device, dtype=torch.long
    )
    src_keypoints1_projected = warp_keypoints(
        src_keypoints1, homography
    )  # keypoint1 must be projected to keypoint2 space for comparsion
    for i in range(
        n_iters
    ):  # 2nd iteration only recover 1 or 2 keypoint matches in most cases. So 1 should be enough.
        ...
        intersect_indexes_2 = torch.where(
            min1[min2] == torch.arange(len(min2), device=min1.device)
        )[0]
        intersect_indexes1 = min2[intersect_indexes_2]
        matched_distances = (
            distance[intersect_indexes1, intersect_indexes_2] < dist_thresh
        )
...
def resize_aspect_ratio(image, resize_h, resize_w):
    ...
    template[
        ((resize_h - resized.shape[0]) // 2) : ((resize_h - resized.shape[0]) // 2)
        + resized.shape[0],
        ((resize_w - resized.shape[1]) // 2) : ((resize_w - resized.shape[1]) // 2)
        + resized.shape[1],
    ] = resized
    return template
